...
D_spread<-read_csv("./msrb_derived_trades_agg_by_month.csv") %>% 
  filter(is.null(yield)==FALSE, yield >0)

D_amihud_f <- read_csv("./msrb_liquidity_variables.csv") %>% 
  select(cusip, year,month, amihud, amihud_risk, roll, irc, irc_risk, disp, trades_per_mo,
         avg_trading_interval,zero_days)%>% 
  mutate(issuer_n = str_sub(cusip, 1,6))

D_spread_cov<-D_spread %>% left_join(D_amihud_f, by = c("cusip", "year","month")) %>% 
  filter(is.na(issuer_n.y)==FALSE) %>% 
  select(cusip, year, month, spread, coupon, dollar_price,maturity_date, yield,issuer_n = issuer_n.y,  amihud, amihud_risk,
          roll,  irc,irc_risk, disp, trades_per_mo,
         avg_trading_interval,zero_days, sec_desc)

D_county_shps <- counties(state="GA") %>% select(GEOID, NAME)

D_amihud_geod_f<- read_csv("./msrb_shape_data.csv") %>% st_as_sf(coords = c("x","y"))

st_crs(D_amihud_geod_f)<-st_crs(D_county_shps)

D_amihud_geod_f_2<-st_join(D_amihud_geod_f, D_county_shps) %>% select(GEOID = GEOID.y, NAME = NAME.y, issuer_n) %>% distinct

#D_initial <-read_csv("~/Desktop/credit_spread_data/D_initial_offering.csv")

D_results_meta_f<- read_csv("./msrb_issuance_meta.csv")

D_ratings_data <-  read_csv("./msrb_credit_ratings.csv")

D_results_meta_f_2<-D_results_meta_f %>% mutate(dated_date = case_when(is.na(as.numeric(dated_date))~ymd(dated_date),
                                                   is.na(as.numeric(dated_date))==FALSE~as_date(dated_date))
                            )
get_rating<-function(fitch, moody,snp){
  x <- c(fitch, moody,snp)
  x[which(x%in%c("none","NR"))]<-"none"
  to_rm <- which(x=="none")
  
  if(length(to_rm>0)){
    x_clean<-x[-which(x=="none")]
  }else{
    x_clean<-x
  }
  
  final_rating<-Mode(x_clean)
  return(final_rating)
}

D_ratings_data_f<-D_ratings_data %>% rowwise %>% 
  mutate(rating_f = get_rating(fitch_long, moody_long, snp_long) %>% ifelse(.=="AAR","AA",.)) %>% 
  select(cusip, rating_f)

#removing this GEOID because it doesn't have any neighbors
D_amihud_final<-D_spread_cov %>% left_join(D_amihud_geod_f_2, by = "issuer_n") %>% st_as_sf %>% 
  data.frame %>% mutate(trade_month = ymd(str_c(year,"-",month,"-01"))) %>% 
select(cusip, trade_month, spread, yield, coupon, dollar_price,maturity_date,
       amihud, amihud_risk, roll, irc, irc_risk, disp, trades_per_mo,sec_desc,
                          avg_trading_interval,zero_days, GEOID) %>% filter(is.na(GEOID)==FALSE) %>% 
  left_join(D_results_meta_f_2, by = "cusip") %>% 
  mutate(initial_offering=principal_amount_at_issuance %>% str_remove(" M") %>% as.numeric) %>% 
  filter(isFALSE(coupon.x==coupon.y)==FALSE) %>% 
  left_join(D_ratings_data_f, by = "cusip")

D_amihud_final_2<-D_amihud_final %>%
  mutate(time_numeric = as.numeric(trade_month))%>%
    rowwise %>% 
  mutate(sec_desc_class=
  case_when(any(str_detect(sec_desc ,c("INNER CITY", "URBAN")))~"inner_city",
            any(str_detect(sec_desc ,c("PAYROLL","TAX")))~"tax",
            any(str_detect(sec_desc ,
                     c("COLLEGE","UNI", "UGA","UNIV GA", "STUDENT", "SCH","KSU","SPORTS", "GA TECH")))~"school",
            any(str_detect(sec_desc ,c("LABOR")))~"labor",
            any(str_detect(sec_desc ,c("PUB SAFETY","PUB SVC")))~"safety",
            any(str_detect(sec_desc ,c("ARPT")))~"airport",
            any(str_detect(sec_desc ,c("HEALTH","HOSP")))~"hospital",
            any(str_detect(sec_desc ,c("WASTE","WTR & SEW", "POLLUTN","WTR & SWR","UTIL","WTR")))~"waste",
            any(str_detect(sec_desc ,c("HSG")))~"housing",
            any(str_detect(sec_desc ,c("PUB FACS", "RECREATIONAL FACS", "DEV AUTH")))~"facilities",
            any(str_detect(sec_desc ,c("SER")))~"series"
)) %>% ungroup %>% mutate(
...
GA_census_tract_data<-get_acs(geography = "county", state = "GA",
                                variables=c(perc_bac = "HC01_EST_VC05", med_inc="B19013_001",white = "B02001_002", 
                                            ...
                                            european_immigrant='B05006_002',
                                            asian_immigrant='B05006_047',
                                            latin_immigrant='B05006_124'
                                ),
                                summary_var = "B01003_001",geometry = TRUE) %>%
    mutate(geometry = st_centroid(geometry)) %>% rowwise %>% 
    mutate(x = str_split(geometry," ")[[1]][1] %>% str_sub(3,nchar(.)-1) %>% as.numeric,
           y = str_split(geometry," ")[[1]][2] %>% str_sub(1,nchar(.)-1) %>% as.numeric) %>% 
    select(GEOID, NAME,variable, summary_est, estimate,x,y) %>%
    data.frame %>%  mutate(perc = estimate/summary_est) %>% 
    pivot_wider(names_from = variable, values_from = c("estimate","perc")) %>% 
    data.frame() %>% select(- geometry)
  
    d_2020<- read_csv("./nyt-us-counties-2020.csv")
    d_2021<- read_csv("./nyt-us-counties-2021.csv")
    d<-rbind(d_2020,d_2021)
    
    D_start_covid<-d %>% filter(state=="Georgia")%>% mutate(GEOID = str_sub(geoid, 5,10)) %>% 
      select(-c(geoid,county,state)) %>% 
      mutate(date = ymd(date), month = month(date), year = year(date)) %>% 
      group_by(month, year, GEOID) %>% summarise(cases = sum(cases)) %>% 
      mutate(trade_month=ymd(str_c(year,"-",ifelse(nchar(month)==1,str_c("0",month), month),"-01"))+35,
             trade_month = str_c((trade_month) %>% str_sub(.,1,8), "01") %>% ymd
             )

    D_tax_data <- read_csv("./18incyallagi.csv")
  
    D_tax_data_2<-D_tax_data %>% mutate(GEOID = str_c(STATEFIPS, COUNTYFIPS),total_inc=N02650*A02650) %>% 
    group_by(GEOID) %>% summarise(total_agi = sum(total_inc)) %>% ungroup %>% 
      mutate(GEOID = as.character(GEOID))
      
  D_rural <- read_csv("./ruca2010revised.csv") %>% filter(`Select State`=="GA") %>% 
    select(GEOID = `State-County-Tract FIPS Code (lookup by address at http://www.ffiec.gov/Geocode/)`,
           ruca = `Primary RUCA Code 2010`, pop_dens = `Population Density (per square mile), 2010` ) %>% 
    mutate(GEOID = str_sub(GEOID, 1,5)) %>% group_by(GEOID) %>% 
    summarise(mode_ruca = Mode(ruca), mean_dens = mean(pop_dens))
      
    
 D_int <- read_csv("./TB4WK.csv")
 
 ##loading issuer data
  
D_sales_type <- read_csv("./D_sales_total_latest.csv") %>% 
  mutate(sales_type = case_when(`Underwriting Spread Amount:`=="Disclosed in Official Statement"~"negotiated",
                                `Underwriting Spread Amount:`!="Disclosed in Official Statement"~"competitive")
         ) %>% select(cusip, sales_type)     

##here we make the issue-level database
d<-D_start_covid %>% mutate(GEOID = as.character(GEOID))

proper_covid<-GA_census_tract_data %>% select(GEOID, NAME) %>% mutate(county = NAME %>% str_remove(" County, Georgia")) %>% left_join(d) %>% select(GEOID, trade_month, I_s =cases)

months<-seq(from=ymd("2018-01-01"),to=ymd("2020-01-01"), by = "days") %>% str_sub(.,1,7) %>% str_c(., "-01") %>% unique

mix_f <- data.frame()

for(i in 1:length(months)){
  mix = cbind(months[i], proper_covid$GEOID %>% unique)
  mix_f <- rbind(mix, mix_f)
}

mix_f_2<-mix_f %>% add_column(I_s=0) %>%mutate(V1 = ymd(V1)) %>% select( GEOID = V2, trade_month = V1, I_s)

proper_covid_f<-rbind(proper_covid, mix_f_2) %>% ungroup %>%
  mutate(trade_month_l1 = (trade_month-days(27)) %>% str_sub(.,1,7) %>% str_c(.,"-01") %>% ymd %>% as.numeric, 
         GEOID = as.numeric(GEOID),
         trade_month_l_1 = (trade_month+days(33)) %>% str_sub(.,1,7) %>% str_c(.,"-01") %>% ymd,
         trade_month = trade_month %>% as.numeric)

GA_census_tract_data_f<-GA_census_tract_data %>% select(GEOID, summary_est, perc_white, perc_black, estimate_med_inc,
                                                        perc_poverty) %>% mutate(GEOID = as.numeric(GEOID))

D_issues<-read_csv("./D_issues_final_latest.csv")
D_ratings_data <-read_csv("./ratings_full_cusip.csv")

D_issuer_cusip <- read_csv("./D_issues_final_latest_conduit.csv") %>% 
  select(cusip_j=CUSIP, callable=Callable, pre_refunded = Pre.refunded,
         taxable=Taxable, conduit = conduit
         )

D_issues_1<-D_issues%>% rowwise %>% mutate(desc = desc %>% str_remove(.,"SERIES 2021") %>%
                     str_remove(., "SERIES 2019") %>% 
                     str_remove(.,"SERIES 2020")) %>% 
  mutate(sec_desc_class=case_when(any(str_detect(desc ,c("INNER CITY", "URBAN")))~"inner_city",
            any(str_detect(desc,c("PAYROLL","TAX")))~"tax",
            any(str_detect(desc ,
                     c("COLLEGE","UNI", "UGA","UNIV GA", "STUDENT", "SCH","KSU","SPORTS", "GA TECH","SCHOOL")))~"school",
            any(str_detect(desc ,c("LABOR")))~"labor",
            any(str_detect(desc ,c("PUB SAFETY","PUB SVC")))~"safety",
            any(str_detect(desc ,c("ARPT","AIRPORT")))~"airport",
            any(str_detect(desc ,c("HEALTH","HOSP","HOSPITAL","HEALTHCARE")))~"hospital",
            any(str_detect(desc ,c("WASTE","WTR & SEW", "POLLUTN","WTR & SWR","UTIL","WTR","WATER")))~"waste",
            any(str_detect(desc ,c("HSG","HOUSING")))~"housing",
            any(str_detect(desc ,c("PUB FACS","PUBLIC FACILITIES", "FACILITIES","RECREATIONAL FACS", "DEV AUTH","DEVELOPMENT","BUILDING","UTILITIES","ECONOMIC")))~"facilities",
            any(str_detect(desc ,c("SER","GENERAL OBLIGATION")))~"series"),
         issue_date = dated_date %>% str_sub(.,1,8) %>% str_c(.,"-01") %>% ymd,
         issue_date_d = dated_date %>% ymd,
         GEOID = as.numeric(GEOID),
         sec_desc = desc,
         issuer_n = str_sub(cusip, 1,6) %>% 
           fct_collapse(
             `047851` = c("047851","04785T","04785V")
           )
  ) %>% ungroup %>% 
  mutate(date_join = issue_date %>% as.numeric) %>% 
  left_join(D_sales_type, by = "cusip") %>% 
  left_join(D_ratings_data_f, by = "cusip" ) %>% 
  mutate(issuer_n)

D_ratings_data_f<-D_ratings_data %>% rowwise %>% 
  mutate(rating_f = get_rating(fitch_long, moody_long, snp_long) %>% ifelse(.=="AAR","AA",.) %>% 
           ifelse(is.na(.), "none", .)) %>%  select(cusip, rating_f) %>% ungroup

B<-proper_covid_f %>% mutate(trade_month_l_1 = as.numeric(trade_month_l_1),
                             trade_month_l1 = as.numeric(trade_month_l1),
                             trade_month = as.numeric(trade_month)
                             )

D_tax_data_2<-D_tax_data_2 %>% mutate(GEOID = as.numeric(GEOID))

D_now<-D_issues_1 %>% left_join(B, by = c("GEOID", "date_join"="trade_month")) %>% distinct %>%
  left_join(D_int %>% mutate(int_date = as.numeric(DATE)), by = c("date_join"="int_date")) %>% 
  left_join(D_issuer_cusip, by = c("cusip"="cusip_j")) %>% 
  left_join(GA_census_tract_data_f, by = "GEOID") %>%
  left_join(D_tax_data_2, by = "GEOID")%>% 
  left_join(D_ratings_data_f, by = "cusip") %>% 
  filter(callable%in%c("No","Yes","none")) %>% 
  mutate(
    sec_desc = as_factor(sec_desc),
    maturity_date = as.numeric(ymd(maturity_date)),
    ttm=(maturity_date-date_join)/365,
    issuer_n = str_sub(cusip, 1,6),
    cusip = as_factor(cusip), 
    I_s = case_when(is.na(I_s)~0, is.na(I_s)==FALSE~I_s),
    true_coupon = principal_amount_at_issuance/(dollar_price)*(coupon)/principal_amount_at_issuance,
    threshold = ifelse(date_join >= as.numeric(ymd("2020-03-18")), 1, 0),
    sec_desc_class = as_factor(sec_desc_class) %>% relevel(.,ref = "series"), 
    GEOID = as_factor(GEOID),
    sales_type = as_factor(sales_type),
    rating_f = as_factor(rating_f.y) %>% relevel(.,ref="Aa"),
    callable = as_factor(callable),
    pre_refunded = as_factor(pre_refunded),
    taxable = as_factor(taxable)) %>% 
  select(true_coupon, principal_amount_at_issuance, price_perc,conduit, perc_poverty,issue_date,estimate_med_inc, maturity_date, dollar_price,ttm, I_s,perc_white, total_agi, TB4WK, sec_desc_class, GEOID, issuer_n, sales_type,rating_f,callable,summary_est,issue_date_d, pre_refunded,taxable,coupon)  %>% na.omit %>% 
  mutate(
    issuer_n = issuer_n %>% as_factor %>% 
      fct_collapse( `434608`=c("A34608","A43460","434608"),
                    `A2308T`=c("A2308T","42308T"),
                    `83306U`=c("83306U","S3306U"),
                    `80036R`=c("80036R","s0036R"),
                    `T76024`=c("T76024","776024"),
                   `7FO47O` = c("7FO47O","7FO470","JFO47O","JFO470","TFO47O","TFOATO","TO4ATO","FOATOR","TFO47T","TFO4A7","JFO4AT"),
                   `705698`=c("JFO569","705698","7O0569","7O5698"),
                   `47407R`=c("47407R","A7T407","A47407","A7407R","AT407R"),
                   `O67869` = c("O67869","os7s69","O06786","067869","o56869","O56869","o56s69","o5s6s6","O05686"),
                   `047870` = c("O47870","047870","o47870"),
                   `047772` = c("047772","O47772"),
                   `04780M` = c("04780M","O4780M","OA7SOM"),
                   `O6962P` = c("O6962P","06962P"),
                   `058083`=c("058083","o58083","o580s3","O58083","o5so0s","O580s3","o5808s"),
                   `047851` = c("047851","04785T","O47851","04785V"),
                   `773043` = c("773043","T73030"),
                   `745425` = c("745425","7A5425"),
                   `704654`=c("7O0465","704654","7O4654"),
                   `703600`=c("703600","7FO360","7O0360","7O3600"),
                   `55554T`=c("55554 ","55554T"),
                   `466782`=c("A46678","466782","A66782"),
                   `426368`=c("A26368","426368"),
                   `426362`=c("A26362","426362"),
                   `A04468` = c("A04468","404468"),
                   `403760`=c("A40376","403760","AO3760","A03760","403 76"),
                   `403720`=c("A40372","403720"),
                   `490806`=c("490806","190806"),
                   `19078V`= c("19078V","419078","49078V"),
                   `O8S86S`=c("O8S86S","OS869F","O8869F","OS86SF","O8S869","O886SF","O8869S"),
                   `22A723`=c("22AT23","22A472","22A723","924723","224723"),
                   `419748`=c("4A9748","419748"),
                   `144753`=c("144753","414475"),
                   `117497`=c("411749","117497","417497"),
                   `117024`=c("117024","411702"),
                   `114366`=c("114366","11436Q"),
                   `068746`=c("068746","O68746"),
                   `058028`=c("058028","o58028"),
                   `046078`=c("046078","o46078","O46078","O04607"),
                   `199144` = c("199144","419914"),
                   `199112` = c("199112","419911")
                   )) %>% 
  mutate(issuer_n = fct_collapse(issuer_n,
                                 `006874`=c("68743","O06874"),
`138186`=c("138186","413818"),
`414478`=c("414478","14478E"),
`144753`=c("144753","4144 7"),
`414927`=c("414927","149278"),
`O4787C`=c("O4787C","04787C"),
`O04780`=c("O04780","04780M","O4780T"),
`O04787`=c("O04787"),
`411307`=c("411307","113074"),
`146441`=c("146441","414644"),
`21288A`=c("21288A","21288 "),
`419082`=c("419082","190828"),
`197493`=c("197493","AST493"),
`419748`=c("419748","197486"),
`19078V`=c("19078V","490806","419080"),
`36005G`=c("36005G","36005F"),
`296117`=c("296117","261172"),
`299147`=c("299147","929914"),
`42225V`=c("42225V","42225Y"),
`704621`=c("704621","7O0462"),
`610550`=c("610550","610548"),
`89051T`=c("89051T","89051 ")
                                 ))
...
We also estimate a series of other models to examine the sensitivity of the inferences to model specification. Our alternatively specified models are a simple linear model, a Poisson GAM with fixed effects, a Gaussian GAM with random effects, a Poisson GAM with mixed effects, a Gaussian GAM with mixed effects and more knots allowed for the smooth terms in the model, as well as a Poisson GAM with mixed effects and a greater number of knots allowed for the smooth terms. 

All of the models, including the linear model, reject the null hypothesis that the COVID coefficient is 0 and whose lower confidence bounds remain not only away from 0, but represent differences that would be relevant in the real world. 
We also present diagnostic plots examining the fit of the Gaussian mixed-effects GAM.
```{r}
...
Let the principal owed on bond i be $p_{i}$, the coupon of that bond be $c_{i}$, and the number of years until maturity $m_{i}$. Cornaggia, sensibly, tells us to calculate the total amount of interest cost for that bond as:
  
$o_{i} = p_{i} \cdot c_{i} \cdot m_{i}$. 
...
Now, let our estimate of the effect of COVID on the coupon rate be $\hat{\beta}_{covid}$ and let $I_{i}$ be number of COVID cases in the month prior to the issuance of bond i. Because our estimate comes from a Poisson model, $e^{\hat{\beta}_{covid}}$ gives us the multiplicative effect of one extra COVID case on the coupon rate of a bond, given on the linear scale. We can use the actual number of COVID cases and this estimate to work backwards from the actual coupon rate for bond i, $c_{a,i}$, to bond i's estimated coupon rate without COVID, $c_{e,i}$ as:

$$
c_{i,e}=c_{i,a}\cdot e^{-\hat{\beta}_{covid} \cdot I_{i}}
$$

giving the new amount of interest due as:

$$
o_{i,e}=c_{i,a}\cdot e^{-\hat{\beta}_{covid} \cdot I_{i}} \cdot p_{i} \cdot m_{i}
$$
...
$$
d_{i} =  o_{i,a}-o_{i,e}\\
 =  p_{i} \cdot m_{i} \cdot (c_{i,a}-c_{i,a}\cdot e^{-\hat{\beta}_{covid} \cdot I_{i}})\\
 =   p_{i} \cdot m_{i} \cdot c_{i,a} \cdot (1-e^{-\hat{\beta}_{covid} \cdot I_{i}})
$$

By summing this quantity up over i, where i indexes the bonds issued post-COVID, we can reach an estimate for the total extra interest cost borne by issuers paying higher coupons post-COVID.
We can also translate the MLE SE into MLE lower and upper confidence bounds for the estimates.
...
If we take the estimates from our Gaussian mixed effects model, we get estimates of the cost of COVID due to increased interest rates between about \$438 million and \$789 million, with a point estimate 
at \$613 million. 
